...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_COINSET] ="images\\Coin_set.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_DICE_VALUE]= "images\\Input.jpg";
	MenuItemImages[ITM_OMOR]= "images\\offer_mor.jpg";
	MenuItemImages[ITM_AMOR]= "images\\accept.jpg";
	MenuItemImages[ITM_RMOR]= "images\\return.jpg";
	MenuItemImages[ITM_SELL]= "images\\sell.jpg";
	MenuItemImages[ITM_NEWGAME]= "images\\restart.jpg";
	MenuItemImages[ITM_SWITCH_TO_DESIGN_MODE] = "images\\Menu_SwitchToGrid.jpg";
	MenuItemImages[ITM_EXITT]= "images\\Menu_Exit.jpg";
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if(playerNum<0 ||playerNum>3){
		string error= "invalid player number";
		PrintMessage(error);
		return ;}
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	{
		string error= "Invalid ladder locations";
		PrintMessage(error);
		return;
	}
	...
	int c=fromCell.VCell()-toCell.VCell();
	int k=0;
	for(int i=0;i<c;i++)
	{
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		int yy=y2+k+(UI.CellHeight/2);
		pWind->DrawLine(x12,yy,x34,yy);
		k=k+UI.CellHeight;	
	}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if(fromCell.HCell()!=toCell.HCell() || fromCell.GetCellNum()<toCell.GetCellNum())
	{
		string error= "Invalid snake locations";
		PrintMessage(error);
		return;
	}
	...
	int px2 = x12 + xChange;
	int py1 = y1 + yChange;
	int py2 = y1 + (2 * yChange);
	int xs[] = { px1,x12,px2,x12 };
	int ys[] = { py1,y1,py1,py2 };
	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
...
{
	int xtopleft= GetCellStartX(cellPos); int ytopleft=GetCellStartY(cellPos);
	///TODO: Write a complete implementation of this function
	image im("images\\MONEY.jpg");
	pWind->DrawImage(im,xtopleft,ytopleft,UI.CellWidth/4,UI.CellWidth/4);
