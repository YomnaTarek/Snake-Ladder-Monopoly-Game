...
public:
	...
	Output * GetOutput() const; // Gets a Pointer to the Output 
	  ...
	  // A setter to be used in copy/cut (in order NOT to break class responsibilities)
	...
	// ========= Other Getters =========
	// ALL FUNCTIONS HERE LOOP OVER CELLLIST AND\OR PLAYERSLIST AND CALLS FUNCTIONS IN THEM OR IN ANY GAME OBJECT NO LOGIC IS INCLUDED 
	Player * GetCurrentPlayer() const;	// Gets a Pointer to the Current Player	                                      
	GameObject * GetNextGame(const CellPosition & position, int object);
    Player*  getplayer(int i);
	int getplayeri(const int &x);
	void setmonopoly(int,int);
	void setmorta(int,int);
	int getmaxcity(int,int&);
	bool checkmortgage(int,int);
	int morta(int);
	Player* isbenefit();
	bool fire(int, int ,int);
	bool evenorodd(int,int);
	Card* check(int x);
	GameObject * getobject(const CellPosition & position);
	GameObject* GetCardOrCoins(const CellPosition & position);
	void SetClipboard(GameObject* GameObj,int);
	int getcopyorcut();
	bool Paste(GameObject* pNewObject, CellPosition position);
	void offer(int,int);
	bool checkoffer(int,int);
	bool checkaccept(int,int);
	void accept(int,int);
	Card* getcity(int);
	void New();
	void SaveAll(ofstream& OutFile, Grid*pGrid, int Type);// Saves all of a certain game object in the grid
	int HowManyGameObj(int Type);//Takes type of game object an dreturn its count in the grid
	Card* CheckCardNumber(int cardnum);
	void SetSave(int cardnum);
	int GetPlayerMax(int walletmax);
